...
bool OyplPortReader::read(yarp::os::ConnectionReader& in)
{
    ...
    {
        gazeboYarpPluginLoaderPtr->addYarpPluginsLists(response);
        return response.write(*out);
    }
    else if ( request.get(0).asString() == "open" ) //-- open
    {
        ...
        if(!gazeboYarpPluginLoaderPtr->Open(sout, sinput))
        {
            response.addVocab32(VOCAB_FAILED);
    ...
    {
        Gazebo::EnvironmentBasePtr penv = gazeboYarpPluginLoaderPtr->GetEnv();
        yarp::os::Value v(&penv, sizeof(Gazebo::EnvironmentBasePtr));
    ...
    else if ( request.get(0).asString() == "close" ) //-- close
    {
        ...
        for(size_t i=1; i<request.size(); i++)
        {
            if(!gazeboYarpPluginLoaderPtr->close(request.get(i).asInt32()))
            {
                response.addVocab32(VOCAB_FAILED);
